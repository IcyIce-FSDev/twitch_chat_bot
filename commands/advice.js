...
async function getRandomAdvice(target, client) {
  ...
  client.say(target, `${randomStarters[rand]}"${advice}"`);
}

module.exports.useAdvice = async (target, context, msg, client) => {
  // This should be a string "!advice"
  const mainCmd = msg.slice(0, 7);

  // Debugging
  if (target == "#monkayshrek") {
    console.log(mainCmd);
  }

  // Gets random slip of advice
  if (mainCmd === "!advice") {
    getRandomAdvice(target, client);
  }
};
